...
class RestaurantOrderManagement:
    ...
    def setup_background(self, root):
        ...
        canvas.pack()
        original_image = tk.PhotoImage(file="C:/Users/Maya/OneDrive - Radix Capital Advisors Private Limited/Desktop/PYTHON/background (1).png")
    ...
    def place_order(self):
        ...
        for item, entry in self.menu_quantities.items():
            ...
            if quantity.isdigit():
                ...
                total_cost += cost
                if quantity > 0:
                    order_summary += f"{item}: {quantity} x {symbol}{price} = {symbol}{cost}/n"
        if total_cost > 0:
            order_summary += f"/nTotal Cost: {symbol}{total_cost}"
            messagebox.showinfo(
                "Order Placed",
